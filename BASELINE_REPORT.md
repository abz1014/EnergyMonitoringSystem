...
| 9 | **`EMS.API` exists, is the intended mobile-app backend, but does not currently build** | Medium-High | `src/EMS.API` is the planned backend for a future mobile app (plant-local-network, data pushed from the system) — not abandoned scaffolding. It has 9 endpoints across 3 controllers (Auth, EnergyAnalysis, Meters), built 2026-06-29, untouched since. **Confirmed via direct build: it fails with 9 compile errors**, all caused by the schema-compliance fixes made to `EMS.Core` models afterward (`DateTime` → `DateTime?`, `int` → `int?`, `double` → `decimal` on PF/kWh fields). `EMS.API`'s controllers were written against the old model shapes and were never updated. **Out of scope for this baseline** (scoped to `EMS.Web`) — needs its own dedicated review and a model-compatibility fix pass before any mobile app work can build on it. Not wired into any `.sln` file currently. |
